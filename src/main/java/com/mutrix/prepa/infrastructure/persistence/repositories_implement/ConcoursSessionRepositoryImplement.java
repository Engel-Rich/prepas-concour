...
import com.mutrix.prepa.domaines.repository_interfaces.ConcoursSessionRepositoryInterface;
...
public class ConcoursSessionRepositoryImplement implements ConcoursSessionRepositoryInterface {
    ...
    public ConcoursSessions createConcoursSession(ConcoursSessions concoursSession) {
        final ConcoursEntity concoursEntity = concoursRepository.findById(concoursSession.getConcoursId())
                .orElseThrow(() -> new RuntimeException("Concours not found with id: " + concoursSession.getConcoursId()));
        final ConcoursSessionEntity concoursSessionEntity = ConcoursSessionEntityMapper.domainToEntity(concoursSession, concoursEntity);
    ...
    public Page<ConcoursSessions> getAllByConcoursId(UUID concoursId, Integer page, Integer size) {
        final Pageable pageable = PageRequest.of(page, size);
        final Page<ConcoursSessionEntity> concoursSessionEntityPage = concoursSessionRepository.findByConcours_Id(concoursId, pageable);
        return concoursSessionEntityPage.map(ConcoursSessionEntityMapper::toDomain);
    }
